...
class Sess: # prefix + prompt + infix + output + suffix
    ...
    def copy(self) -> Self:
        """创建当前会话的深拷贝，返回一个独立的新Sess对象"""
        return type(self)(
            seq=self._seq.copy(),  # 复制序列列表
            prompt_l=self._prompt_l,
            prompt_r=self._prompt_r,
...
def subset_to_indices(subset: Union[int, slice, List[Union[int, slice]]], length: int) -> Set[int]:
    indices = []
    # 把单个 slice 或整数转换为列表
    if isinstance(subset, (int, slice)):
        subset = [subset]
    for item in subset:
        if isinstance(item, slice):
            indices.extend(range(item.start if item.start is not None else 0,
                               item.stop if item.stop is not None else length,
