...
def compute_metrics(len_list: List[Optional[int]], judge_list: List[Optional[float]]) -> dict:
    return dict(
        summary = dict(
        ...
        detail = dict(len_list = len_list, judge_list = judge_list),
    )

SAVE_NAME_TPL = 'o{}.pkl'

def write_metrics(save_dir: str, n_data: int, indices: Optional[Set[int]] = None) -> Tuple[bool, Set[int], List[Optional[int]], List[Optional[float]]]:
    if indices is None:
    ...
    if found:
        with open(osp.join(save_dir, 'metrics.json'), 'w') as fo:
            json.dump(compute_metrics(len_list, judge_list), fo)
    return found, indices, len_list, judge_list
...
def test(gen_cls: type):
    ...
    found, indices, len_list, judge_list = write_metrics(save_dir = save_dir, n_data = env.data.len, indices = subset_to_indices(args.test_subset, env.data.len))
    ...
    for i in tbar:
        ...
        if not osp.exists(save_name):
            ...
            with open(save_name, 'wb') as fo:
                pickle.dump(dict(prompt = gen_sess.prompt, output = gen_sess.output, judge = judge, verbose = verbose), fo)
            env.reset()
            gen.reset()
            metrics = compute_metrics(len_list, judge_list)
            tbar.set_description(f'len={metrics["summary"]["len"]:.2f}, asr={metrics["summary"]["asr"]:.4f}, judge={metrics["summary"]["judge"]:.4f}')
    found, indices, len_list, judge_list = write_metrics(save_dir = save_dir, n_data = env.data.len) # compute metrics for all existing results
